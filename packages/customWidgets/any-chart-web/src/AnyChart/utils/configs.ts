...
export const getConfigOptions = (props: AnyChartProps): Partial<Config> => {
  const parsedConfig: Partial<Config> = JSON.parse(
...
export const getLayoutOptions = (props: AnyChartProps): Partial<Layout> => {
  ...
  const attributeLayout: Partial<Layout> = props.attributeLayout
    ? JSON.parse(props.attributeLayout || "{}")
    : {};

  return deepMerge.all(
    [{ hovermode: "closest" }, staticLayout, attributeLayout],
    { arrayMerge }
  );
};
...
export const getData = (props: AnyChartProps): any[] => {
  ...
  const attributeData: any[] = props.attributeData
    ? JSON.parse(props.attributeData || "{}")
    : {};
