            ...
            if (series.seriesType === "dynamic") {
                if (!series.seriesEntity) {
                    ...
                    if (!(dataEntity === seriesPath || (dataEntityMeta && dataEntityMeta.isReference(seriesPath)))) {
                        errorMessage.push(
                            `'Dynamic series - Series entity ${series.seriesEntity}' in 'series ${
                                index + 1
    ...
    new Promise<SeriesData<S>>((resolve, reject) => {
        if (series.dataEntity) {
            if (series.dataSourceType === "XPath") {
                ...
                parameters.push(
                    ...restParameters.map(
                        parameter => `${parameter.parameterAttribute}=${mxObject.get(parameter.parameterAttribute)}`
                    )
                );
...
export const generateRESTURL = (mxObject: MxO, endpoint: string, parameters: Container.RestParameter[]) => {
    const parameterString = [`contextId=${mxObject.getGuid()}`]
        .concat(
            parameters.map(parameter => `${parameter.parameterAttribute}=${mxObject.get(parameter.parameterAttribute)}`)
        )
        .join("&");
...
export const fetchByXPath = (options: FetchByXPathOptions): Promise<MxO[]> =>
    new Promise<MxO[]>((resolve, reject) => {
        ...
        window.mx.data.get({
            ...
            filter: {
                sort:
                    (sortAttribute && !window.mx.isOffline()) || (sortAttribute && sortAttribute.indexOf("/") === -1)
...
export const fetchByGuids = (options: FetchByIdOptions): Promise<MxO[]> =>
    new Promise<MxO[]>((resolve, reject) => {
        ...
        window.mx.data.get({
            ...
            error: error =>
                reject(`An error occurred while retrieving data for ids (${guids.join(", ")}): ${error.message}`)
        });
    });
