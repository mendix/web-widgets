...
- **Folder**: `bar-chart-web`
- **Category**: Charts
- **Offline capable**: Yes
- **Reference**: [Mendix documentation](https://docs.mendix.com/appstore/widgets/charts#2-4-bar-chart)

## XML Properties

### Data source

- **Series** (`series`; type: object, list). Add one or more lines. The order influences how lines overlay one another: the first line (from the top) is drawn lowest and other lines are drawn on top of it.
...
- **Bar color** (`staticBarColor`; type: expression, optional, data source: staticDataSource). Returns: String.
- **Bar color** (`dynamicBarColor`; type: expression, optional, data source: dynamicDataSource). Returns: String.
...
- **Bar format** (`barmode`; type: enumeration, default: group). Allowed values: `group` (Grouped), `stack` (Stacked).
