## 1. Placement map (`resolveSlots`)

- [x] 1.1 Add a `resolveSlots({ alignment, hasCounter, hasLoadMore, hasPagination })` pure function returning `{ start, middle, end }` with element tokens (`"pagination" | "counter" | "loadMore" | null`).
- [x] 1.2 Implement the rule: alignment names the pagination target slot; pagination takes it if visible; remaining elements go to their natural home (counter → start, Load more → middle) or to the end slot if that home is taken.
- [x] 1.3 Unit-test every alignment × occupancy combination, including `hasPagination: false` (no slot reserved) and the three-element case.
- [x] 1.4 Assert in tests that the end slot is never assigned two elements, documenting why that is structurally impossible.
...
- [x] 3.1 Render `GalleryFooterControls` elements from the `resolveSlots` result instead of fixed per-slot JSX.
- [x] 3.2 Add a `widget-gallery-tb-middle` slot to `GalleryTopBarControls` and render its elements from the same result.
- [x] 3.3 Route custom pagination through the placement map as the pagination element, so alignment moves it too.
- [x] 3.4 Regenerate and review affected component snapshots (new `tb-middle` node, slot reassignment) — review, do not blind-update. (No snapshot updates needed: the only committed snapshot covers `GalleryRoot`, which renders neither bar.)
...
- [x] 5.1 Drive `Gallery.editorPreview.tsx` placement from `resolveSlots`.
- [x] 5.2 Align preview custom pagination with runtime, including the single-render rule for `both`.
- [x] 5.3 Add the `tb-middle` slot to the preview markup so preview and runtime DOM match.
...
- [x] 6.3 `_gallery.scss`: give `fc-middle` / `tb-middle` explicit `display: flex`, `justify-content: center` and flex sizing so Center is centred by construction; add `flex-shrink: 0` where needed to keep slots symmetric.
...
- [ ] 7.3 Verify Center with Load more + Show total count (all three elements).
