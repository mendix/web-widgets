# Rich Text Toolbar Button Specification

## Purpose

Shared `ToolbarDefaultButton` component for the rich text toolbar that renders the underlying `<button>` element for every toolbar control. Consolidates button markup, active/disabled state handling, class composition, ref forwarding, and attribute passthrough so individual toolbar controls no longer hand-roll their own `<button>` elements.
