# List Margin Indent Specification

## Purpose

`Ctrl+]` and `Ctrl+[` apply block indentation via margin. Inside a list, the margin is applied to the nearest list node at any nesting depth (never the inner paragraph); outside a list, it indents the current paragraph, heading, or blockquote. Tab-based list nesting and cursor navigation remain unchanged.
