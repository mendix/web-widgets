# List Tab Indent Specification

## Purpose

Defines how the Tab and Shift+Tab keys behave inside the rich text editor. Inside list items, Tab controls list nesting (sink/lift) rather than margin indentation, while non-list content retains the existing margin-based indentation. Tab interception is scoped to the editor content area so it does not interfere with normal focus navigation on toolbar buttons and other UI elements.
