...
Provide bundled, zero-configuration internationalization for the Rich Text widget so its toolbar, dialogs, and status bar render in the active page language. This capability defines how the active locale is resolved, how keyed translations are looked up with English fallback, the set of bundled languages, the scope of localized UI, and the safe default when no translation provider is mounted.
