...
When one or more `image/*` files are dropped onto the Rich Text editor body and no image source is configured, each valid file SHALL be inserted into the document as an image node whose `src` is a base64 `data:` URI. Insertion SHALL happen at the document position under the drop coordinates, not at the position of the selection before the drop. When several valid image files are dropped at once, they SHALL be inserted in the order they appear in the drop, at that position. Inserted images SHALL NOT be given `width` or `height` attributes; they keep their natural size and remain resizable afterwards. When an image source is configured, dropped files SHALL be routed to the image dialog's entity-upload flow instead of being inserted as base64 images.

#### Scenario: Single image dropped into the editor without an image source

- **WHEN** the user drops a valid image file onto the editor body and no image source is configured
...
#### Scenario: Image dropped away from the cursor without an image source

- **WHEN** the selection is in the first paragraph and the user drops a valid image over the third paragraph while no image source is configured
- **THEN** the image is inserted at the third paragraph, not at the first

#### Scenario: Multiple images dropped at once without an image source

- **WHEN** the user drops three valid image files in a single drop and no image source is configured
- **THEN** three image nodes are inserted in the same order as the dropped files

#### Scenario: Dropped image keeps its natural size without an image source

- **WHEN** a valid image file is dropped and no image source is configured
...
When the clipboard contains one or more `image/*` files and no image source is configured, pasting into the Rich Text editor SHALL insert each valid file as an image node whose `src` is a base64 `data:` URI, at the current selection. The same validation, ordering, and sizing rules as dropped images SHALL apply. When an image source is configured, pasted files SHALL be routed to the image dialog's entity-upload flow instead of being inserted as base64 images.

#### Scenario: Screenshot pasted into the editor without an image source

- **WHEN** the clipboard holds an image file and the user pastes into the editor while no image source is configured
- **THEN** an image node with a base64 `data:` URI `src` is inserted at the selection

#### Scenario: Pasted image replaces the selected range without an image source

- **WHEN** a range of text is selected and an image file is pasted while no image source is configured
