...
(function () {
                    ...
                    function (evt) {
                        ...
                        if (evt.data.keyCode == this.indentKey) {
                            ...
                            if (list) {
                                ...
                                if (
                                    this.isIndent &&
                                    CKEDITOR.plugins.indentList.firstItemInPath(this.context, path, list)
                                )
                                    return;
                ...
                this.jobs[this.isIndent ? 10 : 30] = {
                    refresh: this.isIndent
                        ? function (editor, path) {
                              var list = this.getContext(path),
                                  inFirstListItem = CKEDITOR.plugins.indentList.firstItemInPath(
                                      this.context,
                                      path,
                                      list
                                  );
    ...
    function indentList(editor) {
        ...
        function indent(listNode) {
            ...
            var newList = CKEDITOR.plugins.list.arrayToList(
                listArray,
                database,
                null,
                editor.config.enterMode,
                listNode.getDirection()
            );
            ...
            if (pendingLis && pendingLis.length) {
                for (i = 0; i < pendingLis.length; i++) {
                    ...
                    while (
                        (followingList = followingList.getNext()) &&
                        followingList.is &&
                        followingList.getName() in context
                    ) {
            ...
            if (newList) editor.fire("contentDomInvalidated");
        ...
        while ((range = iterator.getNextRange())) {
            ...
            while (
                nearestListBlock &&
                !(nearestListBlock.type == CKEDITOR.NODE_ELEMENT && context[nearestListBlock.getName()])
            ) {
            ...
            if (
                ...
                range.startContainer.getName() in context
            ) {
            ...
            if (
                ...
                range.endContainer.getName() in context
            ) {
            ...
            if (nearestListBlock) return indent(nearestListBlock);
        }
        return 0;
    }
    ...
    CKEDITOR.plugins.indentList.firstItemInPath = function (query, path, list) {
        ...
        if (!list) list = path.contains(query);
