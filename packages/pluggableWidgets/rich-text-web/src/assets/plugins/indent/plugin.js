...
(function () {
                    ...
                    (function (editor, command) {
                        ...
                        for (var priority in command.jobs) {
                            ...
                            relatedGlobal.on(
                                "refresh",
                                function (evt) {
                                    if (!evt.data.states) evt.data.states = {};

                                    evt.data.states[command.name + "@" + priority] = command.refreshJob(
                                        editor,
                                        priority,
                                        evt.data.path
                                    );
    ...
    CKEDITOR.plugins.indent.specificDefinition.prototype = {
        ...
        refreshJob: function (editor, priority, path) {
            ...
            if (!editor.activeFilter.checkFeature(this)) job.state = TRISTATE_DISABLED;
            else job.state = job.refresh.call(this, editor, path);
