...
import { createElement, ReactElement, useRef, useContext, useCallback } from "react";
...
function ReactiveDatagridTextFilter(props: DatagridTextFilterContainerProps & FilterAPI): ReactElement {
    ...
    const onChange = useCallback(
        (value: string | undefined, type: DefaultFilterEnum): void => {
            store.arg1.displayValue = value ?? "";
            store.filterFunction = type;
        },
        [store]
    );
...
const Widget = withFilterAPI(observer(ReactiveDatagridTextFilter));
