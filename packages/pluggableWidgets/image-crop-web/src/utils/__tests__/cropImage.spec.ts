...
describe("cropImage", () => {
    ...
    test("uses viewport dims as canvas size when outputSize is viewport", async () => {
        ...
        const lastCanvas = (file as any)._lastCanvas as HTMLCanvasElement | undefined;
        if (lastCanvas) {
            expect(lastCanvas.width).toBe(50);
            expect(lastCanvas.height).toBe(40);
        }
    });
...
async function captureDrawImageCalls(fn: () => Promise<unknown>): Promise<any[]> {
    const calls: any[] = [];
