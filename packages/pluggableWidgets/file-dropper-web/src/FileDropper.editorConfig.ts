...
export function getProperties(
    values: FileDropperPreviewProps,
    ...
    if (values.uploadMode === "files") {
        ...
        hidePropertiesIn(properties, values, ["allowedFileFormats", "associatedFiles", "createFileAction"]);
    }

    return properties;
}

export function check(values: FileDropperPreviewProps): Problem[] {
...
// export function getPreview(values: FileDropperPreviewProps, isDarkMode: boolean, version: number[]): PreviewProps {
...
export function getCustomCaption(_values: FileDropperPreviewProps): string {
    return `File dropper (${_values.uploadMode})`;
}
