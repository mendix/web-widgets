    ...
    const items = data.map(item => {
        return columns.map(column => {
            ...
            if (column.showContentAs === "attribute") {
                ...
                //Changed this from the standard implementation to show the attribute for custom content (if the attribute is there)
                value = column.attribute?.get(item)?.displayValue ?? "n/a (custom content)";
            }
