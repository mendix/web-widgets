...
export function extractMultiAssociationProps(props: ComboboxContainerProps) {
    const attr = props.attributeAssociation;
    if (!attr) {
        throw new Error("'optionsSourceType' type is 'association' but 'attributeAssociation' is not defined.");
    }
    if (attr.type !== "ReferenceSet") {
        throw new Error("'attributeAssociation' type is not 'ReferenceSet'.");
    }
